...
export const Generator = () => {
  ...
  const onPickFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!res.ok || !data.ok) {
        toast.error(data.error || t("parseFailed"));
        return;
      }
      ...
      toast.success(
        t("docRead", {
          name: data.name,
          suffix: data.truncated ? t("docTruncated") : "",
        })
      );
    } catch {
  ...
  const onGenerate = async () => {
    ...
    try {
      ...
      if (!res.ok || !res.body) {
        ...
        toast.error(data.error || t("requestFailed", { status: res.status }));
        return;
      }
      ...
      if (acc.includes("[[STREAM_ERROR]]")) {
        toast.error(
          t("streamInterrupted", {
            detail: acc.split("[[STREAM_ERROR]]")[1]?.slice(0, 120) ?? "",
          })
        );
        return;
      }
      ...
      const entry: HistoryEntry = {
        ...
        title: deckTitle(
          deck,
          value.trim() || selectedTemplate?.name || t("untitled")
        ),
        html: deck,
        createdAt: Date.now(),
      };
      ...
      toast.error(e instanceof Error ? e.message : t("genFailed"));
    } finally {
      setGenerating(false);
    }
  };
          ...
          <aside className="fixed top-0 left-0 z-50 h-full w-72 bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl border-r border-white/40 dark:border-white/10 shadow-2xl flex flex-col">
            <div className="flex items-center justify-between p-4 border-b border-white/40 dark:border-white/10">
              <span className="font-semibold flex items-center gap-2">
                <HistoryIcon className="size-4" /> {t("historyTitle")}
              </span>
              <button
                type="button"
                onClick={() => setHistoryOpen(false)}
        ...
        <div className="w-full">
          <div className="flex flex-wrap items-center gap-2 mb-3">
            <Button variant="outline" size="sm" onClick={reset}>
              <RotateCcwIcon className="size-4" /> {t("remake")}
            </Button>
            ...
            <Button variant="outline" size="sm" onClick={openInNewTab}>
              <ExternalLinkIcon className="size-4" /> {t("openNewTab")}
            </Button>
            <Button variant="outline" size="sm" onClick={download}>
              <DownloadIcon className="size-4" /> {t("exportHtml")}
            </Button>
            <Button size="sm" onClick={exportPdf}>
              <FileDownIcon className="size-4" /> {t("exportPdf")}
            </Button>
          </div>
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          {generating && (
            <span className="text-xs text-muted-foreground flex items-center gap-1.5">
              ...
              {t("generating", { chars: streamedChars })}
            </span>
          )}
        </div>
            ...
            {selectedTemplate && (
              <div className="inline-flex items-center gap-2 text-sm bg-blue-600/10 dark:bg-blue-500/20 text-blue-700 dark:text-blue-300 px-3 py-1.5 rounded-lg border border-blue-600/20">
                ...
                <span className="max-w-[220px] truncate">
                  {t("stylePrefix", { name: selectedTemplate.name })}
                </span>
                <button
                  type="button"
